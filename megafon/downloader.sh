#!/bin/bash

set -euo pipefail
set -x


# get from Autonomous System
get_maintained() {
    whois -h whois.ripe.net -- "-i mnt-by $1" | rg '^route' | awk '{ print $2; }'
    whois -h whois.radb.net -- "-i mnt-by $1" | rg '^route' | awk '{ print $2; }'
    whois -h rr.ntt.net -- "-i mnt-by $1" | rg '^route' | awk '{ print $2; }'
    whois -h whois.rogerstelecom.net -- "-i mnt-by $1" | rg '^route' | awk '{ print $2; }'
    whois -h whois.bgp.net.br -- "-i mnt-by $1" | rg '^route' | awk '{ print $2; }'
}

get_maintained 'MEGAFON-RIPE-MNT' | grep -v 'route-set' | grep "/" > /tmp/megafon.txt || echo 'failed'
get_maintained 'MEGAFON-WEST-MNT' | grep -v 'route-set' | grep "/" >> /tmp/megafon.txt || echo 'failed'

# save ipv4
grep -v ':' /tmp/megafon.txt > /tmp/megafon-ipv4.txt

# save ipv6
grep ':' /tmp/megafon.txt > /tmp/megafon-ipv6.txt


# sort & uniq
sort -h /tmp/megafon-ipv4.txt | uniq > megafon/ipv4.txt
sort -h /tmp/megafon-ipv6.txt | uniq > megafon/ipv6.txt
