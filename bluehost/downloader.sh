...
get_routes 'AS46606' > /tmp/bluehost.txt || echo 'failed'

# save ipv4
grep -v ':' /tmp/bluehost.txt | grep -v "0.0.0.0" > /tmp/bluehost-ipv4.txt

# save ipv6
grep ':' /tmp/bluehost.txt > /tmp/bluehost-ipv6.txt

# sort & uniq
sort -h /tmp/bluehost-ipv4.txt | uniq > bluehost/ipv4.txt
sort -h /tmp/bluehost-ipv6.txt | uniq > bluehost/ipv6.txt
