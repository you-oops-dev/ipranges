...
get_maintained 'lir-nl-atom86-1-MNT' > /tmp/atom86.txt || echo 'failed'
get_routes 'AS8455' >> /tmp/atom86.txt || echo 'failed'

# save ipv4
grep -v ':' /tmp/atom86.txt | grep -v "0.0.0.0" > /tmp/atom86-ipv4.txt

# save ipv6
grep ':' /tmp/atom86.txt > /tmp/atom86-ipv6.txt

# sort & uniq
sort -h /tmp/atom86-ipv4.txt | uniq > atom86/ipv4.txt
sort -h /tmp/atom86-ipv6.txt | uniq > atom86/ipv6.txt
