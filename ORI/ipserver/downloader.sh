...
get_maintained 'IP-SERVER-MNT' > /tmp/ipserver.txt || echo 'failed'
get_routes 'AS44812' >> /tmp/ipserver.txt || echo 'failed'

# save ipv4
grep -v ':' /tmp/ipserver.txt | grep -v "0.0.0.0" > /tmp/ipserver-ipv4.txt

# save ipv6
grep ':' /tmp/ipserver.txt > /tmp/ipserver-ipv6.txt

# sort & uniq
sort -h /tmp/ipserver-ipv4.txt | uniq > ORI/ipserver/ipv4.txt
sort -h /tmp/ipserver-ipv6.txt | uniq > ORI/ipserver/ipv6.txt
