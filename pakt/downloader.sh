...
get_maintained 'PAKT-MNT' | grep -v 'route-set' | grep "/" > /tmp/pakt.txt || echo 'failed'


# save ipv4
grep -v ':' /tmp/pakt.txt > /tmp/pakt-ipv4.txt
...
# sort & uniq
sort -h /tmp/pakt-ipv4.txt | uniq > pakt/ipv4.txt
sort -h /tmp/pakt-ipv6.txt | uniq > pakt/ipv6.txt
