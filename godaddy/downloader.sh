...
get_routes 'AS26496' > /tmp/godaddy.txt || echo 'failed'

# save ipv4
grep -v ':' /tmp/godaddy.txt | grep -v "0.0.0.0" > /tmp/godaddy-ipv4.txt

# save ipv6
grep ':' /tmp/godaddy.txt > /tmp/godaddy-ipv6.txt

# sort & uniq
sort -h /tmp/godaddy-ipv4.txt | uniq > godaddy/ipv4.txt
sort -h /tmp/godaddy-ipv6.txt | uniq > godaddy/ipv6.txt
