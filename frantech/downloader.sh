...
get_routes 'AS53667' > /tmp/frantech.txt || echo 'failed'

# save ipv4
grep -v ':' /tmp/frantech.txt | grep -v "0.0.0.0" > /tmp/frantech-ipv4.txt

# save ipv6
grep ':' /tmp/frantech.txt > /tmp/frantech-ipv6.txt

# sort & uniq
sort -h /tmp/frantech-ipv4.txt | uniq > frantech/ipv4.txt
sort -h /tmp/frantech-ipv6.txt | uniq > frantech/ipv6.txt
